...
function Update-Rule($blueTarget, $blueWeight, $greenTarget, $greenWeight) {
    ...
    aws elbv2 modify-rule `
                --rule-arn $rulearn `
                --conditions file://"C:\runner_work\devops\json\conditions_temp.json" `
                --actions file://"C:\runner_work\devops\json\actions_temp.json"
...
$targetHealthDescJson = aws elbv2 describe-target-health --target-group-arn $greentargetgroup
$targetHealthDesc = $targetHealthDescJson | ConvertFrom-Json
$greeninstance = $targetHealthDesc.TargetHealthDescriptions.Target.Id
Write-Output "Green Instance: $greeninstance"
aws elbv2 register-targets --target-group-arn $bluetargetgroup --targets Id=$greeninstance,Port=$port 


# Wait for the green instance to become healthy in the blue target group
do {
    $targetHealth = aws elbv2 describe-target-health --target-group-arn $bluetargetgroup --targets Id=$greeninstance,Port=$port | ConvertFrom-Json
    echo $targetHealth
...
Update-Rule $bluetargetgroup "100" $greentargetgroup "0"

# Add your instance back to the blue target group
echo $bluetargetgroup
aws elbv2 register-targets --target-group-arn $bluetargetgroup --targets Id=$instanceId,Port=$port
